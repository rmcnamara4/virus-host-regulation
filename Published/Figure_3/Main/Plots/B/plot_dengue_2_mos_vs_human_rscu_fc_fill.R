...
rscu_fc_mosquito = read.table(
  ...
  select(-c(virus, realm, locus_tag, host, type, shape, corr, p.value, ATG, TGG, TAG, TAA, TGA))
...
# melt rscu fold change data
rscu_fc_mosquito = melt(rscu_fc_mosquito)
names(rscu_fc_mosquito) = c('codon', 'mosquito_fc')

rscu_fc_human = melt(rscu_fc_human) 
names(rscu_fc_human) = c('codon', 'human_fc')

# merge data
data = merge(opt, rscu_fc_mosquito, by = 'codon')
data = merge(data, rscu_fc_human, by = 'codon')
...
# save
cairo_pdf(
  './dengue_2_mos_vs_human_rscu_fc_fill.pdf', 
  family = 'Arial', 
  height = 7, 
  width = 10
)
