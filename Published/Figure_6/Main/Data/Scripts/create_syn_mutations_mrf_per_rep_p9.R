################################################################################
# script to create MRF of synonymous mutations to each codon per replicate for 
# passage 9
################################################################################
...
syn_mutations_mrf_per_rep = mutation_data %>%
  filter(muttype == 'Syn') %>%
...
syn_mutations_mrf_per_rep$mut_aa = vapply(syn_mutations_mrf_per_rep$mut_aa, function(x) aa_conversion_hash[[x]], character(1), USE.NAMES = FALSE)

# save
write.table(
  syn_mutations_mrf_per_rep, 
  '../syn_mutations_mrf_per_rep_p9.csv', 
